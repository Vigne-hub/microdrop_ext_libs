...
class TaskScheduler(object):
    def __init__(self, task_names):
        self.task_names = list(task_names).copy()
    ...
    def request_order(self, before, after):
        assert before in self.task_names, f"'{before}' not in task names"
        assert after in self.task_names, f"'{after}' not in task names"

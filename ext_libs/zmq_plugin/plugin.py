...
from .hub import get_msg_frames
...
class PluginBase(object):
    ...
    **Thread-safety**
    ...
    def query(self, request, **kwargs):
        ...
        '''
        try:
            self.query_socket.send(json.dumps(request).encode("latin1"))
            reply = json.loads(self.query_socket.recv(**kwargs).decode("latin1"))
            validate(reply)
            return reply
        except:
    ...
    def reset_command_socket(self):
        ...
        self.command_socket.setsockopt_string(zmq.IDENTITY, self.name)
    ...
    def send_command(self, request):
        ...
        self.command_socket.send_multipart(
            [v.encode("latin1") for v in (self.hub_name, '', json.dumps(request))]
        )
    ...
    def _process__execute_request(self, request):
        ...
        self.command_socket.send_multipart(get_msg_frames([self.hub_name, '', reply_str]))
    ...
    def reset_subscribe_socket(self):
        ...
        if self.subscribe_options:
            for k, v in self.subscribe_options.items():
                self.subscribe_socket.setsockopt_string(k, v)
...
def watch_plugin(executor, plugin, wait_duration_s=0.01, callback=None):
    ...
    async def _check_command_socket():
        '''

...
class Transform(SlaveView):
    ...
    def create_ui(self):
        ...
        self.label_tag_transform = Gtk.Label(label = 'Transform: ')
        self.button_rotate_left = Gtk.Button(label = 'Rotate left')
        self.button_rotate_right = Gtk.Button(label = 'Rotate right')
        self.button_reset = Gtk.Button(label = 'Reset')
        self.button_modify = Gtk.CheckButton(label = 'Modify')
...
class VideoInfo(SlaveView):
    def create_ui(self):
        ...
        self.widget.set_orientation(Gtk.Orientation.HORIZONTAL)
        self.label_tag_fps = Gtk.Label(label = 'FPS: ')
        self.label_fps = Gtk.Label()
        self.label_tag_dropped_rate = Gtk.Label(label = '  Dropped/s: ')
        self.label_dropped_rate = Gtk.Label()
        self.widget.pack_start(self.label_tag_fps, False, False, 0) # gkreder
        self.widget.pack_start(self.label_fps, False, False, 0) # gkreder
        self.widget.pack_start(self.label_tag_dropped_rate, False, False, 0) # gkreder
        self.widget.pack_start(self.label_dropped_rate, False, False, 0) # gkreder
...
class VideoSink(SlaveView):
    ...
    def reset(self):
        ...
        status = {'frame_count': 0, 'dropped_count': 0, 'start_time':
                  datetime.now()}
        self.status = status
        self.video_timeout_id = GLib.timeout_add(self._check_duration,
...
if __name__ == '__main__':
    ...
    #GLib.timeout_add(1000, view.debug)

...
# from pygtkhelpers.utils import gsignal # gkreder
...
class ObjectTreeViewBase(Gtk.TreeView):
    ...
    def set_columns(self, columns):
        ...
        for idx, col in enumerate(columns):
            ...
            view_col.pygtkhelpers_objectlist = self
    ...
    def _get_selected_ids(self):
        ...
        model, selected_paths = selection.get_selected_rows()
        if selected_paths:
            return (model.get_path(model.get_iter(path))[0] for path in selected_paths)
        else:
            return ()
    ...
    def _on_query_tooltip(self, objectlist, x, y, ktip, tooltip):
        if not self.get_tooltip_context(x, y, ktip):
            ...
            if hasattr(column, 'pygtkhelpers_column'):
                pcol = column.pygtkhelpers_column
            else:
                pcol = None
    ...
    def _attr_sort_func(self, model, iter1, iter2, attr):
        ...
        return (getattr(obj1, attr, None) > getattr(obj2, attr, None)) - (getattr(obj1, attr, None) < getattr(obj2, attr, None))

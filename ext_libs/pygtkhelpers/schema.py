...
class SchemaDialog(FormViewDialog):
    ...
    def validate(self):
        data_dict = expand_items(flatten_form(self.form_view.form.schema).items())
        errors = OrderedDict([('.'.join(e.path), e)
                              for e in self.validator.iter_errors(data_dict)])

        # Light red color.
        light_red = Gdk.Color(240 / 255., 126 / 255., 110 / 255.)
        ...
        for name_i, field_i in self.form_view.form.fields.items():
            ...
            label_widget_i.get_parent().modify_bg(Gtk.StateType.NORMAL, color_i)

        if errors:
            self.button_ok.set_sensitive(False)
            message = '\n'.join(['[{}] {}'.format(name, error.message)
                                 for name, error in errors.items()])
            self.label_event_box.modify_bg(Gtk.StateType.NORMAL, light_red)
...
class MetaDataDialog(SchemaDialog):
    ...
    def create_ui(self):
        ...
        (self.scanner_window.get_content_area()
         .pack_start(self.scanner_view.widget, True, True, 0))
        self.scanner.start()

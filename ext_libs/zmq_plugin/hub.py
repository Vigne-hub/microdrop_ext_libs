...
def get_msg_frames(msg_list):
    return [get_bit(el) for el in msg_list]
...
class Hub(object):
    ...
    **Thread-safety**
    ...
    def reset_command_socket(self):
        ...
        self.command_socket.setsockopt(zmq.IDENTITY, bytes(self.name.encode()))
    ...
    def query_send(self, message):
        ...
        self.query_socket.send_string(message)
    ...
    def on_query_recv(self, msg_frames):
        ...
        try:
            ...
            self.publish_socket.send_multipart([
                str(request['header']['source']).encode('utf-8'),
                str(request['header']['target']).encode('utf-8'),
                str(request['header']['msg_type']).encode('utf-8'),
                msg_frames[0] if isinstance(msg_frames[0], bytes) else str(
                    msg_frames[0]).encode('utf-8')
            ])
            ...
            self.publish_socket.send_multipart(msg_frames)
    ...
    def on_command_recv(self, msg_frames):
        ...
        '''
        try:
            # convert msg frames to string-literals (str type object) from byte-literals (bytes type object)
            source, null, message_str = [msg.decode('utf-8') for msg in msg_frames]
        except:
    ...
    def _send_command_message(self, message):
        ...
        '''
        # load the message into a string
        message_json = json.dumps(message)

        msg_parts = [
            str(message['header']['target']).encode('utf-8'),
            b'',
            str(message_json).encode('utf-8')
        ]

        self.command_socket.send_multipart(msg_parts)
    ...
    def _process__forwarding_command_message(self, message):
        ...
        *publish* socket.
        ...
        if 'content' in message and not message['content'].get('silent'):
            msg_frames = [str(message['header']['source']).encode('utf'),
                          str(message['header']['target']).encode('utf'),
                          str(message['header']['msg_type']).encode('utf-8'),
                          str(message_json).encode('utf-8')]
        ...
        *command* socket, the message *MUST* be published to the *publish*
        ...
        if message_type == 'execute_request':
            ...
            if not message['content'].get('silent'):
                msg_frames = get_msg_frames([reply['header']['source'],
                                             reply['header']['target'],

...
def add_columns(tree_view, df_py_dtypes, list_store):
    ...
    for column_i, (i, dtype_i) in df_py_dtypes[['i', 'dtype']].iterrows():
        ...
        # if dtype_i in (int, long): # gkreder
        if dtype_i in (int, ):
            property_name = 'text'
        ...
        setattr(cell_renderer_i, 'column_i', i) # gkreder
        setattr(cell_renderer_i, 'column', tree_column_i) # gkreder
...
def set_column_format(tree_column, model_column_index, format_str,
    ...
    def set_property(column, cell_renderer, list_store, iter, store_i):
        value = list_store[iter][store_i.item()]
...
def on_edited_dataframe_sync(cell_renderer, iter, new_value, column,
    ...
    # i, dtype = df_py_dtypes.ix[column_name] # gkreder
    i, dtype = df_py_dtypes.loc[column_name] # gkreder
    # Update the list store with the new value.
    if dtype == float:
        value = si_parse(new_value)
    elif dtype == bool:
        value = not list_store[iter][int(i)]

    if value == list_store[iter][int(i)]:
        # Value has not changed.
        return False
    list_store[iter][int(i)] = value

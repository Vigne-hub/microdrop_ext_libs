...
from . import (svg_polygons_to_df, fit_points_in_bounding_box,
               svg_shapes_to_df, fit_points_in_bounding_box_params)
...
class ShapesCanvas(object):
    ...
    def __init__(self, df_shapes, shape_i_columns, canvas_shape=None,
        ...
        if isinstance(shape_i_columns, (bytes, str)): # gkreder
    ...
    def reset_shape(self, canvas_shape=None, padding_fraction=None):
        ...
        self.df_canvas_tesselations = fit_points_in_bounding_box(self.df_tesselations, canvas_shape, padding_fraction=padding_fraction) # gkreder
        ...
        if self.df_canvas_shapes.shape[0] > 0:
            ...
            self.df_bounding_shapes = pd.DataFrame(
                np.column_stack([bounding_width, bounding_height]),
                columns=['width', 'height'],
                index=df_bounding_coords.index
            )
        else:
            self.df_bounding_shapes = pd.DataFrame(columns=['width', 'height'])
    ...
    def from_svg(cls, svg_filepath, *args, **kwargs):
        ...
        df_shapes = svg_shapes_to_df(svg_filepath) # gkreder

        return cls(df_shapes, 'id', *args, **kwargs) # gkreder
    ...
    def find_shape(self, canvas_x, canvas_y):
        ...
        if hasattr(self.space, 'point_query_first'):
            ...
            # deprecated.
            # if not shape_x.is_integer() and shape_y.is_integer(): # gkreder
                # raise ValueError(f"Expected integers shape_x, shape_y but got ({shape_x}, {shape_y/})")
            info = self.space.point_query_nearest((shape_x, shape_y), 0, [pymunk.ShapeFilter.ALL_CATEGORIES()]) # gkreder

...
class LayerAlphaController(SlaveView):
    ...
    def set_surfaces(self, df_surfaces):
        ...
        cell_renderer.connect('edited', self.on_edited, column,
                              ...
                              self.df_surfaces)
        set_column_format(column, self.df_py_dtypes.loc['alpha'].i, # gkreder
    ...
    def set_alpha(self, surface_name, alpha):
        ...
        # store_alpha_column_index = self.df_py_dtypes.ix['alpha'].i # gkreder
        store_alpha_column_index = self.df_py_dtypes.loc['alpha'].i

        for row in self.list_store:
            if row[int(store_name_column_index)] == surface_name:
                row[int(store_alpha_column_index)] = alpha
                break

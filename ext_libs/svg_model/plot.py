...
def plot_shapes(df_shapes, shape_i_columns, axis=None, autoxlim=True,
    ...
    patches = [Polygon(df_shape_i[['x', 'y']].values, fc=next(props)['color']
                       if color is None else color, **kwargs)
               for shape_i, df_shape_i in df_shapes.groupby(shape_i_columns)] # gkreder

    collection = PatchCollection(patches, match_original=True) # gkreder
...
def plot_shapes_heat_map(df_shapes, shape_i_columns, values, axis=None,
    ...
    collection = PatchCollection(patches.values, cmap=color_map,
                                 norm=norm)
    # collection.set_array(values.ix[patches.index]) # gkreder
    collection.set_array(values.loc[patches.index]) 

...
class NotebookManagerView(SlaveView):
    ...
    def sessions_dialog(self):
        ...
        dialog.get_content_area().pack_start(session_list.widget, True, True, 0)
        return dialog
...
def add_filters(dialog, filters):
    for f in filters:
        ...
        if 'mime_type' in f:
            ...
            # if isinstance(mime_types, types.StringTypes): # gkreder
            ...
            patterns = f['pattern']
            # if isinstance(patterns, types.StringTypes): # gkreder
            if isinstance(patterns, (str,)):
                patterns = [patterns]
            for pattern in patterns:
                print(f'add pattern: "{pattern}"')

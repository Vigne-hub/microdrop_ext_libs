...
#                long: _max('l')} # gkreder
# _DEFAULT_VALUES = {str: '', float: 0.0, int: 0, long: 0L} # gkreder
_MAX_VALUES = {int: _max('i'),
               float: float(2**1024 - 2**971)}
_DEFAULT_VALUES = {str: '', float: 0.0, int: 0}


def gproperty(name, ptype, default=None, nick='', blurb='',
              flags=GObject.ParamFlags.READWRITE, **kwargs):
...
class GObjectUserDataProxy(object):
    ...
    >>> gi.require_version("Gtk", "3.0")
    >>> from gi.repository import Gtk
    >>> from microdrop.ext_libs import GObjectUserDataProxy
    >>> w = Gtk.Label()
...
class XFormatter(string.Formatter):
    ...
    def __init__(self, *lookup_objects, **extra_converters):
        ...
        if 'e' not in extra_converters:
            extra_converters['e'] = html.escape  #_xml_escape
